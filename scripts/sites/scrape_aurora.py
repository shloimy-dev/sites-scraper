#!/usr/bin/env python3
"""
Aurora scraper. Strategy: Shopify search by UPC/name → follow first product link → JSON-LD.
auroragift.com uses Shopify.
"""
import sys, time
...
def scrape_product(page, upc, name):
    for query in [upc, name]:
        if not query:
            continue
        url = f"{BASE}/search?type=product&q={quote_plus(query)}"
        ...
        if data.get("title"):
            ...
            pl, pw, ph = get_piece_dimensions_from_page(html, data.get("description", ""))
            if pl or pw or ph:
                data["piece_length"] = pl
                data["piece_width"] = pw
                data["piece_height"] = ph
            return data

    return None


def get_piece_dimensions_from_page(html, description):
    """Try to extract dimensions from page."""
    pl, pw, ph = extract_dims_from_html(html)
        ...
        for i, row in enumerate(rows):
            ...
            try:
                ...
                if data:
                    ...
                    data.setdefault("piece_length", pl)
                    data.setdefault("piece_width", pw)
                    data.setdefault("piece_height", ph)
                    results.append(data)
