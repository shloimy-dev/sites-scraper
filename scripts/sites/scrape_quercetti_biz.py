#!/usr/bin/env python3
"""
Quercetti Biz scraper. Extract description, image_url, and dimensions from quercettistore.com.
Sheet: Quercetti Biz.csv. Base: https://www.quercettistore.com
Shopify site. Search: /search?q=QUERY. Product URLs: /products/pallino
"""
import re
import sys
import time
...
def find_product_links(page):
    ...
    for el in page.query_selector_all("a[href*='/products/']"):
        ...
        if href not in seen and "quercettistore.com" in href:
            seen.add(href)
...
def find_best_product_link(page, name):
    ...
    for href, text in links:
        ...
        if name_lower.replace(" ", "") in slug or slug in name_lower.replace(" ", ""):
...
def scrape_product(page, upc, name):
    """Search quercettistore.com and return product data (description, image_url, dimensions)."""
    queries = [name, upc] if name else [upc]
