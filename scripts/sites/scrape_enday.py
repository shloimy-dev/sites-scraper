#!/usr/bin/env python3
"""
Enday scraper. Strategy: Shopify search-suggest API.
Cloudflare blocks the browser but the search/suggest.json API works.
Returns targeted search results for each product name.
"""
...
def is_good_match(query, result_title):
    ...
    common_filler = {"the", "a", "and", "with", "by", "of", "enday", "emraw", "pack", "ct"}
    ...
    if not qw_sig:
        return False
    return len(overlap) / len(qw_sig) >= 0.35
