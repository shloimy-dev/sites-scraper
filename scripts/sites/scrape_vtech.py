#!/usr/bin/env python3
"""
VTech scraper. Strategy: Shopify search by name/UPC → follow first product link → JSON-LD.
Reference: vtechkids.com
"""
import sys, time
...
def scrape_product(page, upc, name):
    for query in [name, upc]:
        if not query:
            continue
        for search_path in ["/search?q=", "/search?type=product&q="]:
            url = f"{BASE}{search_path}{quote_plus(query)}"
            ...
            # Reject category pages (e.g. "VTech Baby Monitors, Soothers & Accessories")
            t = data.get("title", "")
            if t and "soothers & accessories" not in t.lower():
                data["upc"] = upc
