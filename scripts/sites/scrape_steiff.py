#!/usr/bin/env python3
"""
Steiff scraper. Strategy: Search by UPC/name → follow product link.
NOTE: deep_investigate found CANNOT SCRAPE - search returns results page but no product links.
...
def find_first_product_link(page):
    for sel in PRODUCT_SELECTORS:
        ...
        if el:
            ...
            if "/product" in href:
                if href.startswith("/"):
                    href = "https://www.steiff.com" + href
...
def scrape_product(page, upc, name):
    for query in [upc, name]:
        ...
        if data.get("title") and "search results" not in (data.get("title") or "").lower():
            data["upc"] = upc
