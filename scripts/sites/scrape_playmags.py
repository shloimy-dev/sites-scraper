#!/usr/bin/env python3
"""
Playmags scraper. Strategy: Shopify search by name/UPC → follow first product link → JSON-LD.
...
def scrape_product(page, upc, name):
    for query in [name, upc]:
        ...
        for search_path in ["/search?q=", "/search?type=product&q="]:
            ...
            if data.get("title") and "403" not in data.get("title", ""):
                data["upc"] = upc
