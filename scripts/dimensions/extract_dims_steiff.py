#!/usr/bin/env python3
"""
Extract dimensions for steiff.com products.
Fetch product_url, extract dimensions from JSON-LD or HTML. Uses dim_base.
"""
import argparse
import re
import sys
...
def extract_steiff_dims(html, description):
    """Steiff-specific: Size: 17 in, measures 17 inches long, 17\"."""
...
def main():
    ap = argparse.ArgumentParser(description="Extract dimensions for steiff.com products")
    ...
    if not rows or not path:
        print("No steiff.csv found in data/extracted or data/ready/extracted")
        sys.exit(1)
    ...
    for row in rows:
        ...
        pl, pw, ph = extract_steiff_dims(html, desc)
