#!/usr/bin/env python3
"""
Extract dimensions for thelittleluxury.com (Winfun) products.
Load data/extracted/winfun.csv, fetch product_url, extract dimensions from JSON-LD or HTML.
"""
import argparse
import sys
import time
...
def main():
    ap = argparse.ArgumentParser(description="Extract dimensions from thelittleluxury.com (Winfun) product pages")
    ...
    if not rows or not path:
        print("No winfun.csv found in data/extracted or data/ready/extracted")
        sys.exit(1)
