#!/usr/bin/env python3
"""
Extract piece dimensions (L/W/H) from microkickboard.com product pages.
Uses data/extracted/microkick.csv or data/ready/extracted/microkick.csv.
Fetches product_url, extracts from JSON-LD or HTML, updates piece_length/width/height.
"""
import argparse
import time
...
def main():
    ap = argparse.ArgumentParser(description="Extract dimensions from microkickboard.com product pages")
    ...
    if not rows or not path:
        print("No CSV found for microkick (data/extracted/microkick.csv or data/ready/extracted/microkick.csv)")
        return 1
