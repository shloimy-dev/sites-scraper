#!/usr/bin/env python3
"""
Extract dimensions for metalearth.com products.
Fetch product_url HTML, extract from JSON-LD or HTML. Uses dim_base.
Note: metalearth.com may return 403 (Cloudflare) for automated requests.
"""
import argparse
import sys
import time
...
def main():
    ap = argparse.ArgumentParser(description="Extract dimensions for metalearth.com products")
    ...
    if not rows or not path:
        print("No metal_earth.csv found in data/extracted or data/ready/extracted")
        sys.exit(1)
