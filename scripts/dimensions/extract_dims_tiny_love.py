#!/usr/bin/env python3
"""
Extract dimensions for tinylove.com products.
1. First try parse_dims_from_desc(description)
2. For rows still missing: fetch product_url, extract from JSON-LD or HTML
"""
import argparse
import sys
import time
...
def main():
    ap = argparse.ArgumentParser(description="Extract dimensions for tinylove.com products")
    ...
    if not rows or not path:
        print("No tiny_love.csv found in data/extracted or data/ready/extracted")
        sys.exit(1)
